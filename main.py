#Darling Rodriguez
#12/07/2021
#Intro to Programming- 06
#Final Project: Bank of Lux
...
def WasteT():
    ...
    for char in "Please don't waste our time and exist the premises\n":
...
def start():
    ...
    counter3 = 0
    while True:
        greeting = input("Good day, Wecome to Bank of Lux, are you a pre-existing customer here? Y/N \n")
        if greeting == 'Y' or greeting == 'y' or greeting == 'yes' or greeting == 'Yes':
            for char in "Welcome back, pleasure to have you again with us at Bank of Lux. Please, continue forward:\n":
                print(char, end='', flush=True)
                time.sleep(.02)
            break
        elif greeting == 'n' or greeting== 'N' or greeting=='no' or greeting =='No':
            for char in "Well, I welcome you to our bank and I hope that you make a right choice by sticking with us. Please, continue forward:\n":
                print(char, end='', flush=True)
                time.sleep(.02)
            break
        else:
    ...
    while True:
        ...
        forward1 = input('')
        if forward1 == "A" or forward1 == "a":
            for char in "Great, Its right down the hall:\n":
                print(char, end='', flush=True)
                time.sleep(.027)
            accountant()
            break
        elif forward1 == "B" or forward1 == "b":
            for char in "Thank you for coming to Bank of Lux:\n":
                print(char, end='', flush=True)
                time.sleep(.027)
            sys.exit("Simulation Over!")
        elif forward1 == "C" or forward1 == "c":
            for char in "Welcome President, for security reason, what is the access code? \n":
                print(char, end='', flush=True)
                time.sleep(.027)
            while True:
                ...
                if Access == '06172021':
                    owner()
                    break
                elif len(Access)>= 9 or len(Access)< 8:
                    print("The code you have entered is invalid, please try again.")
                    counter2 += 1
                elif Access.isdigit()==False:
                    print("The code you have entered is invalid, please try again with numbers only.")
                    counter2 += 1
                else:
                    print("The code you have entered is invalid, please try again.")
                    counter2 += 1
...
def owner():
    ...
    while True:
        ...
        if desc == "A" or desc == "a":
            ...
            for char in "Thank you for coming to Bank of Lux, Have an awesome day!!! :)\n":
                print(char, end='', flush=True)
                time.sleep(.027)
            sys.exit("Simulation Over!")
        else:
...
def MonQuantity():
    with open("output.csv") as fin:
        ...
        for row in csv.reader(fin):
            total += float(row[7])
        for char in "President, There is a total of $", round(total,2), " dollars in Bank of Lux.\n":
            print(char, end='', flush=True)
            time.sleep(.027)
        owner()
...
def accountant():
    ...
    while True:
        ...
        if newAccount == 'Y' or newAccount == 'y' or newAccount == 'yes' or newAccount == 'Yes':
            for char in "Great, I am glad you chose us, please fill out this required form for me to get you in our system:\n":
                print(char, end='', flush=True)
                time.sleep(.027)
            application() #Application Function
            break
        elif newAccount == 'n' or newAccount == 'N' or newAccount =='no' or newAccount =='No':
            for char in "Okay, Would you like to check your account balance or deposit/widthdraw money? Y/N \n":
                print(char, end='', flush=True)
                time.sleep(.027)
            accountNeed = input('')
            while True:
                if accountNeed == 'Y' or accountNeed == 'y' or accountNeed == 'yes' or accountNeed == 'Yes':
                    money() #money function
                elif accountNeed == 'n' or accountNeed == 'N' or accountNeed =='no' or accountNeed =='No':
                    for char in "Okay, Well thats all the services we offer here at the moment, have a great day!\n":
                            print(char, end='', flush=True)
                            time.sleep(.027)
                    sys.exit("Simulation Over!")
                else:
            ...
            if whatName in Names:
                ...
                if whatSSN == theSSN:
                    ...
                    if str(theDate2) == str(today):
                        print("Hello, Welcome to you account, Your Current Balance is: " , theMoney)
                        deposM()
                        break
                    else:
                        ...
                        print("Hello, Welcome to you account, Your Current Balance is: " , afterA)
                        deposWi() 
                else:
                    print("Sorry, the SNN provided is not correct to the one in the account. Try Again.")
                    counter1 +=1
                    if counter1 >= 4:
                        WasteT()
            else:
                print("Sorry, the name provided doesn't seem to be in our system. Do you Want to create an account with us today? Y/N")        
                WrongName= input('')
                if WrongName == 'Y' or WrongName == 'y' or WrongName == 'yes' or WrongName == 'Yes':
                    for char in "Great, I am glad you chose us, please fill out this required formed for me to get you in our system: \n":
                        print(char, end='', flush=True)
                        time.sleep(.027)
                    print('\n' * 2)
                    application() #Application Function
...
def application():
    ...
    with open('CustomerInfo.csv') as csvfile:    ###Used to make sure the SSN inputed isn't already used
        readCSV = csv.reader(csvfile, delimiter=',')
        SSNs = []
        for col in readCSV:
            SSNa = col[3]
    ...
    for char in "Welcome to the Application of new customers of Bank of Lux, Please fill everything correctly below: \n":
        print(char, end='', flush=True)
        time.sleep(.027)
    ...
    while True:
        ...
        bdayCustomer = str(input('What is your Birthday? (Must be in YYYY-MM-DD Format) \n'))
        dateCustomer = str(datetime.date.today().strftime('%Y-%m-%d'))
        InAmountCustomer = float(input('What is the Initial Amount Being deposited? \n'))
        ...
        if not isValidDate:
            ...
            print("Sorry, the SSN or ITIN you provided already has been used to form an account, please check again to see if you got it corrrect. Else, contact a higher up, Thank you.")
            counter3 += 1
            if counter3 >=4:
                WasteT()
        elif len(NCheck)<=1:
        ...
        elif 100000 <= socialCustomer < 999999 and 1 <= int(bdD) <= 31 and 1 <= int(bdM) <= 12 and 1920 <= int(bdY) <= 2022 and InAmountCustomer >= 0:
            def age(birthdate):
                today = date.today()
            ...
            if realA != ageCustomer:
                print("Sorry, Your Provided age doesn't seem to math with your birthdate, try again")
                counter4 += 1
                if counter4 >=4:
                    WasteT()
            elif realA < 18:
                print("Sorry, Your Provided age means you are not old enough to make an account.")
                counter5 += 1
                if counter5 >=4:
                    WasteT()
            else:
                with open('CustomerInfo.csv', 'a',newline='' ) as File:
                    ...
                    writer.writerow([Name.lower(),ageCustomer,genderCustomer.lower(),socialCustomer,bdD,
                         bdM,bdY,InAmountCustomer, dateCustomer])
                File.close()
                break
        else:
            print("Sorry, Your information is not correct, please redo the application in the correct format")
            counter2 += 1
            if counter2 >=4:
                WasteT()
...
def deposM():
    ...
    while True:
        ...
        if moneyDe == "A" or moneyDe == "a":
            ...
            for char in "Thank you for being with us at Bank of Lux, Have an Awesome Day! \n":
                print(char, end='', flush=True)
                time.sleep(.027)
            sys.exit("Simulation Over!")
        else:
